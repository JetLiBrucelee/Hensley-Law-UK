...
router.post("/contact", async (req, res) => {
  ...
  try {
      ...
      html: `
        <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; color: #1a1a2e;">
          ...
          <div style="padding: 16px 32px; background: #1a1a2e; text-align: center;">
            <p style="color: #666; font-size: 11px; margin: 0;">Submitted via theodorehensleylegal.com — Regulated by the SRA</p>
          </div>
        </div>
      `,
    });
    ...
    res.status(500).json({ success: false, error: "Failed to send enquiry. Please try again." });
  }
});
